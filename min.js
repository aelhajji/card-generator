...
var perimg = document.getElementById('per-img-html')
...
var perimghtml = document.getElementById('per-img-html')
...
// by @elhajji 
function changeImg() {
    const perimg = document.getElementById('per-img').files[0];
    console.log(perimg)
    if (perimg) {
        this.perimghtml.className = ""
        const reader = new FileReader();
        reader.addEventListener('load', function () {
            perimghtml.setAttribute("src", this.result);
        });
        reader.readAsDataURL(perimg);
    }
}
